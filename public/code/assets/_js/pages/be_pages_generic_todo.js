...
class pageTodo {
  ...
  static taskAdd(taskContent) {
    ...
    taskList.prepend(`
      <div class="js-task block block-rounded mb-2 animated fadeIn overflow-hidden" data-task-id="${taskIdNext}" data-task-completed="false" data-task-starred="false">
        <table class="table table-borderless table-vcenter mb-0">
          <tr>
            <td class="text-center pe-0" style="width: 38px;">
              <div class="js-task-status form-check">
                <input type="checkbox" class="form-check-input" id="tasks-cb-id${taskIdNext}" name="tasks-cb-id${taskIdNext}">
                <label class="form-check-label" for="tasks-cb-id${taskIdNext}"></label>
              </div>
            </td>
  ...
  static taskSetActive(taskId) {
    ...
    if (task.length > 0) {
      ...
      task.toggleClass('bg-body-light');
